...
class GraphLSTM_VAE(nn.Module, PyTorchUtils):
    def __init__(self, nodes_num: int, input_dim: int, hidden_dim: int,
                 ...
                 batch_first: bool = True, bias: tuple = (True, True), variational: bool = True, seed: int = 0,
                 gpu: int = None):
        super().__init__()
    ...
    def forward(self, ts_batch, edge_index, use_teacher_forcing=True):
        ...
        if self.variational:
            mu = self.deal_list(self.mu,
                                enc_hidden)  # self.mu(enc_hidden)#self.deal_batch(self.mu, enc_hidden, edge_index)#self.mu(enc_hidden, edge_index)
            logvar = self.deal_list(self.logvar,
                                    enc_hidden)  # self.logvar(enc_hidden)#self.deal_batch(self.logvar, enc_hidden, edge_index)#self.logvar(enc_hidden, edge_index)
        ...
        output[ts_batch.shape[0] - 1] = self.deal_list(self.hidden2output,
                                                       enc_hidden)  # self.hidden2output(enc_hidden)#self.deal_batch(self.hidden2output, enc_hidden, edge_index)#self.hidden2output(enc_hidden, edge_index)
        ...
        if self.variational:
            ...
            output_logvar[ts_batch.shape[0] - 1] = self.deal_list(self.hidden2output_logvar,
                                                                  enc_hidden)  # self.hidden2output_logvar(enc_hidden)#self.deal_batch(self.hidden2output_logvar, enc_hidden, edge_index)
            ...
            for i in reversed(range(ts_batch.shape[0] - 1)):
                ...
                output_logvar[i] = self.deal_list(self.hidden2output_logvar, dec_hidden[-1][
                    0])  # self.hidden2output_logvar(dec_hidden[-1][0])#self.deal_batch(self.hidden2output_logvar, dec_hidden[-1][0], edge_index)
        else:
            for i in reversed(range(ts_batch.shape[0] - 1)):
                if self.training and use_teacher_forcing:
                    _, dec_hidden = self.decoder(ts_batch[i + 1].unsqueeze(0).float(), edge_index, dec_hidden)
