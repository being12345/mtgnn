...
class GraphLSTM_VAE_AD(Algorithm, PyTorchUtils):
        ...
        if self.variational:
            for epoch in range(self.num_epochs):
                ...
                for (i, ts_batch) in enumerate(tqdm(train_loader)):
                    ...
                    total_loss, recon_loss, kl_loss = self.lstmed.loss_function(output, self.to_var(ts_batch.float()),
                ...
                if valid_loss < best_val_loss:
                    ...
                    torch.save(self.lstmed.state_dict(),
                               self.name + '_' + self.kind + str(self.gpu) + '_' + 'checkpoint.pt')
                    counter = 0
                else:
                    counter += 1
    ...
    def get_time_sequence(self, data, nodes_num, selected_indexes):
        ...
        :param data: ndarray
            shape: (time_num, microservice) in MMS table this shape is: (-1, 350)
        :param nodes_num: int
            microservice_num
        :param selected_indexes: int
            select batch_size
        :return: list
            shape: (batch_size, sequence_length(known as timestep), nodes_num, performance_metrics_num)
            in MMS table this shape is: (batch_size, sequence_length, 50, 7)
        """
                ...
                for (i, ts_batch) in enumerate(tqdm(data_loader)):
                    ...
                    for j in range(sampling_num):
                        ...
                        error_origin = torch.div((output - self.to_var(ts_batch.float())) ** 2,
                                                 output_logvar.exp()) + output_logvar
