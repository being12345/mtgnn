...
class ClModel:
    ...
    def trainer(self, train_loader, valid_loader):
        ...
        for epoch in range(n_epochs):
            self.train_model(epoch, n_epochs, train_loader)

            mean_valid_loss = self.valid_model(epoch, n_epochs, valid_loader)
    ...
    def valid_model(self, epoch, n_epochs, valid_loader):
        valid_loss_record = []
        for x, y in tqdm(valid_loader):
            x, y = x.to(self.train_config['device']), y.reshape(y.shape[0] * y.shape[1], -1).to(self.train_config['device'])
        ...
        valid_criterion_record = [criterion.calculate_mean_and_clear() for criterion in self.valid_criterion_set]
        self.log.epoch_valid_info(epoch, n_epochs, mean_valid_loss, valid_criterion_record)
        return mean_valid_loss

    def train_model(self, epoch, n_epochs, train_loader):
        ...
        for step, (x, y) in enumerate(tqdm(train_loader)):
            x, y = x.to(self.train_config['device']), y.reshape(y.shape[0] * y.shape[1], -1).to(self.train_config['device'])  # Move your data to device.
        ...
        train_criterion_record = [criterion.calculate_mean_and_clear() for criterion in self.train_criterion_set]
